...
1. **Multi-continent AXL mesh + signed cross-validation, anchored on-chain.** Frankfurt + NYC, both AXL nodes PM2-wrapped, peered over TLS:9001 on Yggdrasil since 2026-04-24 (TLS link evidence: commit [`777cc08`](https://github.com/smartflowproai-lang/quorum/commit/777cc08), `infra/axl-hello.sh`, `logs/d1-axl-mesh-live.log`, Frankfurt pubkey `68d7077e…`, NYC inbound peer recorded). The application-layer signed roundtrip evidence is the 2026-05-02 verdict attestation TX [`0x19bb1d0e...e1763f22`](https://basescan.org/tx/0x19bb1d0eb990de5152c753e185cd44bca3bf7445abafa982132263a0e1763f22) — calldata contains a canonical evidence-hash payload signed independently by Frankfurt Judge ed25519 + NYC Verifier ed25519, both pubkeys embedded in payload, both signatures publicly verifiable on-chain. Anchored on Base mainnet block 45,476,871. Two-continent cross-validation, immutable, decode docs in `logs/d10-quorum-attestation-tx.json`.

2. **Two on-chain mainnet receipts — not testnet screenshots.** *Verdict attestation* (primary anchor): tx [`0x19bb1d0e...e1763f22`](https://basescan.org/tx/0x19bb1d0eb990de5152c753e185cd44bca3bf7445abafa982132263a0e1763f22), block 45,476,871 — 0-value calldata-only TX anchoring an ed25519-signed canonical evidence hash from Frankfurt Judge + NYC Verifier (decode format in `logs/d10-quorum-attestation-tx.json`). *Treasurer swap* (Uniswap anchor): tx [`0xc03b8350...79b49849`](https://basescan.org/tx/0xc03b8350c982c805e5e2b4aa072fb69138e26c2364b7a70c3ef3b34079b49849), block 45,300,516 — 1 USDC → WETH via Universal Router + Permit2. Both from Treasurer wallet `0xd779cE46…58C893` (EIP-7702 smart EOA, Pectra set-code delegate, production-grade account abstraction). Both verified via `eth_getTransactionByHash` against `mainnet.base.org`, chainId 8453.
