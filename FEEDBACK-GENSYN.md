...
Most AXL integration feedback is one builder, one box, one weekend. Mine is filtered through (a) a real two-continent deployment — Frankfurt VPS running Scout + Judge, NYC VPS running Verifier + Executor + Treasurer, both AXL nodes PM2-wrapped, peered over TLS:9001 on Yggdrasil since 2026-04-22 — and (b) an x402 observatory I've been running outside the hackathon: 22,056 endpoints catalogued across the three primary x402 registries plus tail sources, 6.41M raw Base mainnet x402 payment candidates indexed over a rolling 18-day window, 14.5% currently facilitator-classified (the rest mid-backfill against Base RPC; methodology in `DATA-COVERAGE.md`).
...
**What happened.** First attempt: silent. Frankfurt node reported "listening", NYC node reported "listening", neither logged a peer connection nor a connection attempt failure. Took me 25 minutes to realise the NYC droplet had `ufw` blocking 9001 outbound by default — the AXL node's `Connected outbound` log line never fires because the SYN never leaves the box, and there's no equivalent of "I am trying to reach peer X but cannot" on a periodic cadence. Once 9001 was opened the handshake completed in under 2 seconds, both directions, and the log line `Connected inbound` / `Connected outbound` appeared (now visible in `infra/axl-hello.sh` step 3). The asymmetry that bit me: the listening side is loud, the dialling side is silent.
...
**What happened.** Median was within budget — measured p50 was 138 ms over a 30-minute clean window, which lines up. The tail was the surprise. During simulated bursts (Judge emitting at 12-15/s for 10-second windows, modelled on the burst patterns I see in observatory data — clean Base x402 payment volume averaging ~168K/day with the top facilitator-class signing addresses clustering in 5-second emit windows worth ~4-5x baseline), p95 climbed to 480 ms and p99 hit 1.4 s. The latency cliff lined up exactly with the inbox growth pattern from item 2 — the bound isn't compute (ed25519 is trivial) and isn't network (RTT is stable), it's framing/serialisation queueing on the receiver. Treasurer's reservation window was set to 500 ms on the assumption of a calmer p95; I bumped it to 2 s after observing the tail.
...
- The **bidirectional Frankfurt ↔ NYC roundtrip** worked first try once the firewall was open. Sub-2s handshake, stable peering, no flapping over the 10-day build window. For a fresh cloud-VPS pair across continents, that's the right shape.
...
The grounded view from running an x402 observatory outside the hackathon: the agent-economy traffic shape is real, growing, and bursty in ways naive sizing doesn't anticipate. 6.41M Base mainnet x402 payment candidates over 18 days is not theoretical demand — it's already on-chain. The integrators who'll matter to AXL over the next 18 months are not the ones writing single-host demos — they're the ones running multi-region meshes against tens of thousands of signed-envelope roundtrips/day. Items 3, 5, and 6 are written from inside that future.
