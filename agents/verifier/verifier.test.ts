...
test('pollLoop: non-string envelope.data dropped without crash (Run #3 bug_007 regression)', async () => {
  ...
  _resetRateForTests();
  const sent: Array<{ peer: string; payload: unknown }> = [];
  // Buffer.byteLength throws on non-string input — guard must catch BEFORE the
...
// ---------------------------------------------------------------------------
// Long-lived per-host ed25519 keypairs — sign-verify roundtrip across hosts
// ---------------------------------------------------------------------------
//
// Replaces the ephemeral-keys footnote in the README ("What's rough" #1).
// The Frankfurt Judge and NYC Verifier each own a long-lived ed25519 key
// generated by infra/deploy-vps.sh and stored under agents/{judge,verifier}/
// keys/. Pubkeys are committable; secret keys are gitignored and chmod 600.
//
// Tests below use ephemeral fixtures (NOT the production secret keys) so the
// suite is hermetic — CI never sees a real .sec. The fixtures exercise the
// exact same loader code path used in production, validating:
//   1. Load .pub + .sec from disk, sign with Frankfurt sec, verify with
//      Frankfurt pub. Asserts asymmetricKeyType=ed25519 + keypair self-check.
//   2. Cross-host attestation shape: build canonical evidence bytes mirroring
//      decode-attestation-tx.mjs's QUORUMV1 calldata layout, sign with
//      Frankfurt Judge + NYC Verifier independently, verify both sigs against
//      the DER SPKI pubkeys embedded in the payload. This is the exact shape
//      that lands on Base mainnet.

async function writeFixtureKeyPair(dir: string, baseName: string): Promise<void> {
  await fs.mkdir(dir, { recursive: true });
  const { publicKey, privateKey } = generateKeyPairSync('ed25519');
  await fs.writeFile(path.join(dir, `${baseName}.pub`), publicKey.export({ format: 'pem', type: 'spki' }));
  await fs.writeFile(path.join(dir, `${baseName}.sec`), privateKey.export({ format: 'pem', type: 'pkcs8' }), {
    mode: 0o600,
  });
  await fs.chmod(path.join(dir, `${baseName}.sec`), 0o600);
}

test('host-keys: Frankfurt Judge keypair loads + signs + verifies (long-lived key roundtrip)', async () => {
  const fixDir = path.join(TEST_ROOT, 'judge-keys');
  await writeFixtureKeyPair(fixDir, 'host-frankfurt');

  const kp = await loadHostKeyPair({
    keysDir: fixDir,
  ...
  assert.equal(kp.privateKey.asymmetricKeyType, 'ed25519');
  assert.match(kp.publicKeyDerHex, /^[0-9a-f]+$/);
  // SPKI DER for ed25519 is 44 bytes = 88 hex chars; the prefix
  // 302a300506032b6570032100 is invariant across keys.
  assert.equal(kp.publicKeyDerHex.length, 88);
  assert.ok(kp.publicKeyDerHex.startsWith('302a300506032b6570032100'));

  const payload = Buffer.from('canonical-evidence-hash-from-judge-on-frankfurt');
  const sig = signWithHostKey(kp, payload);
  assert.equal(sig.length, 64, 'ed25519 sig must be 64 bytes');
  assert.equal(verifyWithHostKey(kp, payload, sig), true);

  // Tampered payload must fail verification.
  const tampered = Buffer.from('canonical-evidence-hash-from-judge-on-frankfurt!');
  assert.equal(verifyWithHostKey(kp, tampered, sig), false);
});

test('host-keys: Frankfurt Judge + NYC Verifier cross-host attestation sign-verify roundtrip (decode-attestation-tx.mjs shape)', async () => {
  // Provision both host keypairs in tmp fixtures.
  const judgeDir = path.join(TEST_ROOT, 'judge-keys-2');
  const verifierDir = path.join(TEST_ROOT, 'verifier-keys-2');
  await writeFixtureKeyPair(judgeDir, 'host-frankfurt');
  await writeFixtureKeyPair(verifierDir, 'host-nyc');

  const judgeKp = await loadHostKeyPair({
    keysDir: judgeDir,
    baseName: 'host-frankfurt',
    role: 'judge',
    host: 'frankfurt',
  });
  const verifierKp = await loadHostKeyPair({
    keysDir: verifierDir,
    baseName: 'host-nyc',
    role: 'verifier',
    host: 'nyc',
  ...
  // Build the same canonical evidence payload shape that lands on Base mainnet
  // via decode-attestation-tx.mjs. Verbatim field set, in matching order.
  const ts = new Date().toISOString();
  const evidence = {
    protocol: 'QUORUM',
    version: '1.0',
    case_id: 'quorum-attestation-roundtrip-test',
    claim:
      '5-agent mesh cross-validation roundtrip: Frankfurt Judge + NYC Verifier independently signed the canonical evidence hash, both verifiable via decode-attestation-tx.mjs.',
    evidence_hash: createHash('sha256').update('quorum-roundtrip-test ' + ts).digest('hex'),
    judge_pubkey_hex: judgeKp.publicKeyDerHex,
    verifier_pubkey_hex: verifierKp.publicKeyDerHex,
    judge_role: 'frankfurt',
    verifier_role: 'nyc',
    verdict: 'PASS',
    ts,
  };
  const payloadBytes = Buffer.from(JSON.stringify(evidence), 'utf8');

  // Frankfurt Judge signs.
  const judgeSig = signWithHostKey(judgeKp, payloadBytes);
  // NYC Verifier signs the same canonical bytes (independent attestation).
  const verifierSig = signWithHostKey(verifierKp, payloadBytes);

  assert.equal(judgeSig.length, 64);
  assert.equal(verifierSig.length, 64);
  // Independent attestations — sigs must differ (different private keys).
  assert.notEqual(judgeSig.toString('hex'), verifierSig.toString('hex'));

  // Verify against the embedded pubkey hexes — this is exactly what
  // decode-attestation-tx.mjs does after reading calldata off Base mainnet.
  const judgePubFromPayload = require('node:crypto').createPublicKey({
    key: Buffer.from(evidence.judge_pubkey_hex, 'hex'),
    format: 'der',
    type: 'spki',
  });
  const verifierPubFromPayload = require('node:crypto').createPublicKey({
    key: Buffer.from(evidence.verifier_pubkey_hex, 'hex'),
    format: 'der',
    type: 'spki',
  });
  assert.equal(cryptoVerify(null, payloadBytes, judgePubFromPayload, judgeSig), true);
  assert.equal(cryptoVerify(null, payloadBytes, verifierPubFromPayload, verifierSig), true);

  // Sig swap must fail — Judge's sig cannot verify under Verifier's pubkey
  // and vice versa. Catches accidental key-mixup in the attestation builder.
  assert.equal(cryptoVerify(null, payloadBytes, judgePubFromPayload, verifierSig), false);
  assert.equal(cryptoVerify(null, payloadBytes, verifierPubFromPayload, judgeSig), false);

  // Assemble QUORUMV1 calldata exactly as attestation-tx.mjs does and
  // round-trip parse it to confirm decode-attestation-tx.mjs compatibility.
  ...
  assert.equal(calldata.slice(0, 8).toString('utf8'), 'QUORUMV1');
  const parsedLen = calldata.readUInt16BE(8);
  assert.equal(parsedLen, payloadBytes.length);
  const parsedPayload = calldata.slice(10, 10 + parsedLen);
  const parsedJudgeSig = calldata.slice(10 + parsedLen, 10 + parsedLen + 64);
  const parsedVerifierSig = calldata.slice(10 + parsedLen + 64, 10 + parsedLen + 128);
  assert.deepEqual(parsedPayload, payloadBytes);
  assert.deepEqual(parsedJudgeSig, judgeSig);
  assert.deepEqual(parsedVerifierSig, verifierSig);

  // Final end-to-end check — re-verify both sigs against the re-parsed
  // calldata bytes. If decode-attestation-tx.mjs runs against a TX containing
  // this calldata, both signatures will resolve VALID.
  const reParsedVerdict = JSON.parse(parsedPayload.toString('utf8'));
  const reParsedJudgePub = require('node:crypto').createPublicKey({
    key: Buffer.from(reParsedVerdict.judge_pubkey_hex, 'hex'),
    format: 'der',
    type: 'spki',
  });
  const reParsedVerifierPub = require('node:crypto').createPublicKey({
    key: Buffer.from(reParsedVerdict.verifier_pubkey_hex, 'hex'),
    format: 'der',
    type: 'spki',
  });
  assert.equal(cryptoVerify(null, parsedPayload, reParsedJudgePub, parsedJudgeSig), true);
  assert.equal(cryptoVerify(null, parsedPayload, reParsedVerifierPub, parsedVerifierSig), true);
});
