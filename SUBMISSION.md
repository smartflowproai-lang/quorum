...
|---|---|---|
| 11 live KH `call_workflow` executions vs `app.keeperhub.com` MCP + 62 stub iterations vs local harness | [`logs/d6-keeperhub-wire-verify.log`](./logs/d6-keeperhub-wire-verify.log) (96 KB) | 11 live executions confirm wire works against production KH endpoint (read-only `zwarm-test-sepolia-balance-check` workflow, price=0); 62 stub iterations validate client-side idempotency replay/retry shape (`keeperhub-wire/idempotency.ts`). Latency 1-3 s clean / 8-12 s with transient backoff |
