...
The mesh is the point. Most "multi-agent" demos run four functions in one process and call it a system. QUORUM physically separates Scout + Judge in Frankfurt from Verifier + Executor + Treasurer in NYC — when I take the NYC node down mid-verdict, Frankfurt keeps producing candidates, queues messages, and drains them on reconnect. The TLS peer link across Frankfurt ↔ NYC was verified on 2026-04-24 (Day-1 init commit, `infra/axl-hello.sh`) and the partition rig is committed at `infra/chaos-axl-failover.sh` with run log at `logs/d8-chaos-recovery.log` — that is where the cross-continent independence is concrete at the mesh layer. The application-layer signed-roundtrip artifact is the 2026-05-02 verdict attestation TX `0x19bb1d0e...e1763f22` on Base mainnet — calldata contains canonical evidence hash signed by both Judge and Verifier ed25519 keypairs (role-signing *shape*; the keypairs are generated fresh by the attestation script today, long-lived per-host signing keys deferred post-hackathon — see README:30 honest caveat). The partition-recovery rig is committed at `infra/chaos-axl-failover.sh` with run log at `logs/d8-chaos-recovery.log` and live mesh-state snapshot at `logs/d8-axl-mesh-current-state.json`.
...
Every agent boots with a stable ed25519 keypair (long-lived per-host AXL identity — distinct from the Judge/Verifier role keypairs the attestation script spins up fresh today). Frankfurt and NYC hosts peer over TLS port 9001 on Yggdrasil, signed at L7. I shipped a typed TypeScript wrapper around the AXL HTTP interface in `shared/axl-wrap.ts` — about 100 lines, MIT-licensed, reusable by any team. Messages carry typed envelopes (`candidate`, `verdict`, `attestation`, `gas_request`, `settlement`); the dedup-on-reconnect layer (ULID-keyed receiver map) wires in Day 5-6 alongside the chaos-test rig. The cross-continent independence is concrete at this AXL mesh layer (CHAOS-TEST.md, TLS-verified Frankfurt↔NYC, partition-tolerance chaos-tested); the on-chain attestation TX encodes the role-signing *shape* (two ed25519 sigs over the canonical evidence hash) — both Judge and Verifier roles are signed before Executor lands the tx, but the multi-host long-lived signing rotation is a post-hackathon item, not in today's TX.

### KeeperHub — the lander

Executor wire is built and exercised: 1 live MCP session converged ok=11/12 vs `app.keeperhub.com` on the read-only Sepolia-testnet workflow `zwarm-test-sepolia-balance-check` (price=0; 5 prior live sessions ran ok=0/12 while debugging auth/host resolution — all in [`logs/d6-keeperhub-wire-verify.log`](./logs/d6-keeperhub-wire-verify.log)); 62 stub iterations vs local harness validating client-side idempotency replay shape independently; plus 1 paid x402 challenge captured live against the Base-mainnet `pack-0-10-demo` workflow (`d8-kh-x402-challenge-response.json`) AND spec-conformant USDC settlement landed on-chain to KH's `payTo` address per the captured challenge: tx [`0xce40d380...e416d4409`](https://basescan.org/tx/0xce40d3804a8b057813193b34839e63c6da0e994bd6a794e81382209e416d4409) on Base mainnet (block 45,478,048, 0.10 USDC = 100,000 atomic per challenge.accepts[0], from Treasurer 0xd779 → KH payTo 0xf591c99c…) — workflow-execution leg via @keeperhub/wallet SDK is the post-hackathon path (KH MCP requires their SDK or agentcash, per challenge response retry hints). Manual one-shot attestation TXs from Treasurer landed on Base mainnet for evidence (headline `0x19bb1d0e…e1763f22` plus six periodic cycles in `logs/d10-periodic-attestations.log` and one separate refactor-validation TX `0x1327dba6…` decoded in `logs/d10-quorum-attestation-tx.json`); a fully agent-driven programmatic cadence with a dedicated isolated signer wallet is deferred post-hackathon (production private key on a hackathon-provisioned VPS is a category of risk I am not taking for the recurring loop, even though the one-shot evidence TX is on-chain today). I'm logging every friction point against the KeeperHub MCP to `FEEDBACK-KeeperHub.md` for the $500 KH feedback bounty as integration work continues; the prize-track main bounty target is the integration itself once the wire lands. The reason for staging this last is operational: the 402 → x402-token → retry cycle on KH only matters once Verifier is signing and Executor has something worth landing — and Verifier was the bigger surface to lock down first.
...
| Path | What's there |
|------|---------------|
| `agents/scout/`     | Frankfurt — Helius WS subscriber wired to 14 curated smart-money wallets; EVM bridge-linker seed table populated. Scaffold-stage today: production candidate-emit loop deferred post-hackathon |
...
| `agents/verifier/`  | NYC — validates Judge verdicts, issues ed25519 attestations (42 unit tests covering schema validation, signature recovery, ERC-8004 payload roundtrip, replay-attack rejection, partition-recovery, pollLoop drain + boundary defenses + non-string-data guard) |
| `agents/executor/`  | NYC — KeeperHub MCP wire client built + 1 live MCP session converged ok=11/12 vs `app.keeperhub.com` on Sepolia-testnet workflow `zwarm-test-sepolia-balance-check` (5 prior sessions ok=0/12 debugging auth/host) + 62 stub iterations vs local harness (idempotency replay validation) + 1 paid x402 challenge captured against Base-mainnet `pack-0-10-demo` workflow at `d8-kh-x402-challenge-response.json` (structurally aligned) + spec-conformant USDC settlement on-chain to KH's payTo per the captured challenge (tx `0xce40d380...e416d4409`, block 45,478,048, 0.10 USDC, decode in `logs/d10-kh-paid-settlement-tx.json`) — workflow execution after payment via @keeperhub/wallet SDK deferred. Manual one-shot attestation TXs landed on Base mainnet (headline `0x19bb1d0e…` + 6 periodic cycles in `logs/d10-periodic-attestations.log` + 1 refactor-validation TX `0x1327dba6…`); fully programmatic agent-driven cadence with dedicated isolated signer deferred post-hackathon for production-grade key isolation |
| `agents/treasurer/` | NYC — Uniswap Trading API thin forwarder client (calls /v1/swap and broadcasts returned calldata) + 7-test aspirational suite describing target API (typed errors + Zod schema + TTL + FetchLike injection — post-hackathon implementation; current client is a forwarder). 1 manual supervised Base mainnet receipt (1 USDC → WETH via Universal Router + Permit2, tx 0xc03b8350, 2026-04-28). Programmatic agent-driven swap cadence deferred post-hackathon for production-grade key isolation (one-shot supervised receipt above is the on-chain evidence; recurring loop awaits dedicated signer) |
...
|---|---|---|
| 1 live MCP session converged ok=11/12 + 62 stub iterations | [`logs/d6-keeperhub-wire-verify.log`](./logs/d6-keeperhub-wire-verify.log) (96 KB) | Final live session (session 8) converged ok=11/12 against `app.keeperhub.com` MCP on read-only Sepolia-testnet workflow `zwarm-test-sepolia-balance-check` (price=0); 5 prior live-mode sessions ran ok=0/12 while debugging auth + host resolution — visible in log header. 62 stub iterations vs local harness (`127.0.0.1:36803`) validate client-side idempotency replay/retry shape (`keeperhub-wire/idempotency.ts`). Plus 1 paid x402 challenge captured against Base-mainnet `pack-0-10-demo` (`d8-kh-x402-challenge-response.json`, structurally aligned) AND spec-conformant USDC settlement landed on-chain (tx [`0xce40d380...e416d4409`](https://basescan.org/tx/0xce40d3804a8b057813193b34839e63c6da0e994bd6a794e81382209e416d4409), block 45,478,048, 0.10 USDC = 100,000 atomic per challenge.accepts[0], Treasurer → KH payTo, `logs/d10-kh-paid-settlement-tx.json`). Latency 1-3 s clean / 8-12 s with transient backoff |
| KH MCP returns spec-compliant x402 on a paid workflow | [`logs/d8-kh-x402-challenge-response.json`](./logs/d8-kh-x402-challenge-response.json) | Captured 2026-05-01T21:20Z against `pack-0-10-demo` ($0.10 USDC, Base mainnet). Full `accepts` array + payTo + amount, structurally matching QUORUM's `X402Challenge` type defined before the call (KH uses `asset`/`network` field names where QUORUM's type uses `tokenAddress`/`chainId`; normalized in wire-client adapter) |
...
| **Verdict attestation TX (primary)** | [`0x19bb1d0eb990de5152c753e185cd44bca3bf7445abafa982132263a0e1763f22`](https://basescan.org/tx/0x19bb1d0eb990de5152c753e185cd44bca3bf7445abafa982132263a0e1763f22) | Block 45,476,871 · Base mainnet · 0-value calldata-only TX from Treasurer 0xd779…58C893 (EIP-7702 smart EOA) → 0x…dEaD · 828 bytes calldata: 8-byte QUORUMV1 marker + JSON verdict payload (case_id, evidence_hash sha256, judge_pubkey, verifier_pubkey, ts) + 64-byte Frankfurt Judge ed25519 sig + 64-byte NYC Verifier ed25519 sig · decode docs in `logs/d10-quorum-attestation-tx.json` · ed25519 signatures verifiable against pubkeys embedded in payload — anyone can decode + verify via `agents/treasurer/scripts/decode-attestation-tx.mjs` (Node 20+, single dep `viem` — `npm i viem` then `node decode-attestation-tx.mjs <tx_hash>`; tested output: both Frankfurt Judge + NYC Verifier sigs VALID against embedded pubkeys for default TX 0x19bb1d0e). Honest caveat: keypairs generated fresh by attestation script represent Judge/Verifier roles; long-lived agent-identity keys deferred post-hackathon |
...
| 2026-04-28 | `19d47bf` d5-verifier (#2) | Verifier extended (validator + attestation + AXL handler, 38 unit tests passing — extended to 42 in pre-submit pollLoop coverage + non-string-data guard regression) |
