...
**What happened.** I haven't yet logged realised-vs-estimated gas across burst vs non-burst windows in this build window — `keeperhub-wire/mcp-client.ts` is `callWorkflow`/`searchWorkflows` only and the bundled-estimator integration lands Day 6-7 per `SUBMISSION.md`. So this is a forward concern, not a measured drift. Mechanically I expect a recent-block-mean estimator to under-estimate during the bursty windows my observatory shows (clean payment burst events of several multiples of baseline volume; Base's mempool / priority-fee signal during those windows is genuinely degraded), exactly when an autonomous agent is least able to absorb a "wallet ran dry mid-job" failure.
